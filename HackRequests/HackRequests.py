...
def extract_dict(text, sep, sep2="=")->Dict[str,str]:
  """根据分割方式将字符串分割为字典
  Args:
    text: 分割的文本
    sep: 分割的第一个字符 一般为'\n'
...
class httpcon(object):
  ...
  def _make_con(self, scheme, host, port, proxy=None)-> Union[client.HTTPConnection,client.HTTPSConnection]:
    if "https" != scheme:
      if proxy:
...
class hackRequests(object):
  ...
  def __init__(self, conpool=None):
    ...
    self.httpcon = conpool or httpcon(timeout=17)
  ...
  def httpraw(self, raw: str, **kwargs):
    ...
    if redirect and location:
      ...
      return self.http(redirect, data=None, method=method, headers=headers, location=True, locationcount=1)

    return response(rep, _url, log, )

  def http(self, url:str, *, data:Union[str, Dict[str,str]]="", json:Union[str,List,Dict]="", method:Literal["GET","POST","HEAD"]="GET", location:bool=True, locationcount:int=0, proxy:Union[None,Tuple[str,int]]=None, headers:Union[str,Dict[str,str]]="", cookie:Union[str,Dict[str,str]]="", referer:str="", user_agent:str="", real_host:str="", **kwargs):
    """
    url: 用于传递一个地址
    data: 用于传递post提交，此参数被选择时，method自动变为POST
    json: 用于传递json提交, 此参数被选择时, 自动覆盖post参数
    method: 访问模式，支持三种HEAD、GET、POST
    location: 当状态码为301、302时会自动跳转
    proxy: 代理，需要传入一个tuple，类似 ('127.0.0.1',8080)
    user_agent: 若为空则会模拟一个正常的请求头
    ...
    if data and not json:
      ...
      if not isinstance(data, str):
        try:
          ...
          raise HackError("json parse error")
      default_headers["Content-type"]="application/json"
    tmp_headers = dict(default_headers, **headers)
    assert isinstance(data,str)
    
    try:
      conn.request(method, path, data, tmp_headers)
      rep = conn.getresponse()
      # body = rep.read()
    except socket.timeout:
      raise HackError("socket connect timeout")
    ...
    location = rep.msg.get('location', None)  # handle 301/302
    if location and locationcount < 10:
      if not location.startswith('http'):
        location = parse.urljoin(url, location)
      return self.http(location, data="", method=method, headers=tmp_headers, location=True,
               locationcount=locationcount + 1)

    if not location:
      location = url
    log["url"] = location
    return response(rep, location, log, cookie)
...
def httpraw(raw: str, **kwargs)->response:
  # con = httpcon(timeout=timeout)
